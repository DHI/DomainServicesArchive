namespace DHI.Services.Scalars
{
    ...
    public class ScalarServiceConnection<TId, TFlag> : BaseConnection where TFlag : struct
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="ScalarServiceConnection{TId, TEntity}" /> class.
        /// </summary>
        /// <param name="id">The identifier.</param>
        /// <param name="name">The name.</param>
        public ScalarServiceConnection(string id, string name)
            : base(id, name)
        {
        }
        ...
        public static ConnectionType CreateConnectionType<TConnection>(string path = null) where TConnection : ScalarServiceConnection<TId, TFlag>
        {
            var connectionType = new ConnectionType("ScalarServiceConnection", typeof (TConnection));
            connectionType.ProviderTypes.Add(new ProviderType("RepositoryType", ScalarService<TId, TFlag>.GetRepositoryTypes(path)));
        ...
        public override object Create()
        {
            try
            {
                ...
                {
                    return new ScalarService<TId, TFlag>((IScalarRepository<TId, TFlag>)repository);
                }

                var loggerType = Type.GetType(LoggerType, true);
                var logger = Activator.CreateInstance(loggerType, LoggerConnectionString);
                return new ScalarService<TId, TFlag>((IScalarRepository<TId, TFlag>)repository, (ILogger)logger);
            }

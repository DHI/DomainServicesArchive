namespace DHI.Services.Scalars
{
    ...
    public class GroupedScalarService<TId, TFlag> : BaseGroupedUpdatableDiscreteService<Scalar<TId, TFlag>, TId>, IGroupedScalarService<TId, TFlag> where TFlag : struct
    {
        private readonly IScalarRepository<TId, TFlag> _repository;

        /// <summary>
        ///     Initializes a new instance of the <see cref="GroupedScalarService{TId, TFlag}" /> class.
        /// </summary>
        /// <param name="repository">The repository.</param>
        /// <param name="logger">The logger.</param>
        public GroupedScalarService(IGroupedScalarRepository<TId, TFlag> repository, ILogger logger = null)
            : base(repository)
        {
            if (!(logger is null))
            {
        ...
        public override bool TryUpdate(Scalar<TId, TFlag> scalar, ClaimsPrincipal user = null)
        {
            try
            {
                var scalarBefore = _repository.Get(scalar.Id).Value;
                if (scalarBefore.Locked)
                {
                    return false;
                }
        ...
        {
            return Service.GetProviderTypes<IGroupedScalarRepository<TId, TFlag>>();
        }
        ...
        {
            return Service.GetProviderTypes<IGroupedScalarRepository<TId, TFlag>>(path);
        }
        ...
        {
            return Service.GetProviderTypes<IGroupedScalarRepository<TId, TFlag>>(path, searchPattern);
        }
    ...
    /// <inheritdoc />
    public class GroupedScalarService<TFlag> : GroupedScalarService<string, TFlag> where TFlag : struct
    {
        /// <inheritdoc />
        public GroupedScalarService(IGroupedScalarRepository<string, TFlag> repository, ILogger logger = null)
    ...
    /// <inheritdoc />
    public class GroupedScalarService : GroupedScalarService<int> 
    {
        /// <inheritdoc />
        public GroupedScalarService(IGroupedScalarRepository<string, int> repository, ILogger logger = null)
            : base(repository, logger)
        {
        }
    }

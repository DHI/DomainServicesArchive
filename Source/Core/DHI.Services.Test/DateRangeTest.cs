namespace DHI.Services.Test
{
    ...
    public class DateRangeTest
    {
        ...
        [Fact]
        public void DateRangeWithJsonSampleDeserializer()
        {
            var _options = new JsonSerializerOptions
            {
                PropertyNamingPolicy = JsonNamingPolicy.CamelCase,
            };
            _options.Converters.Add(new DateRangeConverter()); // Customize Converter for DateRange

            var jsonDtRangeSample = "{\r\n    \"from\": \"2014-01-01T00:00:00\",\r\n    \"to\": \"2014-01-02T12:00:00\"\r\n  }";

            var jsonDtRangeDeserialize = JsonSerializer.Deserialize<DateRange>(jsonDtRangeSample, _options);

            Assert.Contains("01/01/2014", jsonDtRangeDeserialize.From.ToString("MM/dd/yyyy")); 
            Assert.Contains("01/02/2014", jsonDtRangeDeserialize.To.ToString("MM/dd/yyyy"));
        }

        [Fact]
        public void DateRangeSerializeDeserializer()
        {
            var _options = new JsonSerializerOptions
            {
                PropertyNamingPolicy = JsonNamingPolicy.CamelCase,
            ...
            var dtRangeVal = new DateRange(DateTime.Now, DateTime.Now.AddDays(1));

            var SerlizeDtRange = JsonSerializer.Serialize(dtRangeVal, _options);

            var DerlizeDtRange = JsonSerializer.Deserialize<DateRange>(SerlizeDtRange, _options);

            Assert.Contains(DateTime.Now.ToShortDateString(), DerlizeDtRange.From.ToString());
            Assert.Contains(DateTime.Now.AddDays(1).ToShortDateString(), DerlizeDtRange.To.ToString());
        }
    }
}
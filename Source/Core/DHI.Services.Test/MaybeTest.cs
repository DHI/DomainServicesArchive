namespace DHI.Services.Test
{
    ...
    public class MaybeTest
    {
        ...
        public void PipeOperatorIsOk()
        {
            ...
            var emptyNullableDouble = Maybe.Empty<double?>();
            Assert.Null(emptyNullableDouble | default(double?));
            Assert.Null(emptyNullableDouble | (double?)null);
            ...
            var maybeNullableDouble = new Maybe<double?>(9.9);
            Assert.Equal(9.9, maybeNullableDouble | 5.5);

            var maybeBoolean = new Maybe<bool>(true);
            Assert.True(maybeBoolean | false);

            var maybeEntity = new Maybe<FakeEntity>(new FakeEntity("id", "name"));
            var entity = maybeEntity | default(FakeEntity);
            Assert.Equal("id", entity.Id);

            maybeEntity = Maybe.Empty<FakeEntity>();
            entity = maybeEntity | default(FakeEntity);
            Assert.Null(entity);
        }
    }
}
namespace DHI.Services.GIS.Maps
{
    ...
    /// <summary>
    ///     Map Source decorator supporting file based caching.
    /// </summary>
    /// <seealso cref="IMapSource" />
    public class FileCachedMapSource : BaseMapSource
    {
        ...
        /// <summary>
        ///     Initializes a new instance of the <see cref="FileCachedMapSource" /> class.
        /// </summary>
        /// <param name="mapSource">The original map source to decorate.</param>
        /// <param name="parameters">The parameters.</param>
        public FileCachedMapSource(IMapSource mapSource, Parameters parameters)
        {
            ...
            var cacheRoot = parameters.GetParameter("CacheRoot", null);
            _cache = cacheRoot == null ? new FileCache(new ObjectBinder()) : new FileCache(cacheRoot, new ObjectBinder());
            _cacheItemPolicy = new CacheItemPolicy {SlidingExpiration = TimeSpan.FromMinutes(parameters.GetParameter("CacheExpirationInMinutes", int.MaxValue))};
        }
